#
# @lc app=leetcode.cn id=5789 lang=python3
#
# [5789] 你完成的完整对局数
#
# https://leetcode-cn.com/problems/the-number-of-full-rounds-you-have-played/description/
#
# algorithms
# Medium (42.39%)
# Likes:    4
...
class Solution:
    def numberOfRounds(self, st: str, ft: str) -> int:
        s=60*int(st[:2])+int(st[3:])
        f=60*int(ft[:2])+int(ft[3:])
        if f<s:f+=1440
        f=(f//15)*15
        return max(0,(f-s))//15
# @lc code=end
